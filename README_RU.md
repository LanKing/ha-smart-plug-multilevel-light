...
2. Не умный светильник, который после включения в розетку включается автоматически и для полного удовольствия имеет несколько режимов яркости.
...
> Моя розетка не умеет сообщать в HA показания автоматически и позволяет только Polling (переопрос розетки с интервалом). По умолчанию он был 60 секунд. Необходимо установить этот интервал на 1 секунду, иначе вы будете очень долго ждать переключения режимов. 

_Данное описание актуально только для владельцев розеток подключенных через Z2M, другим пользователям следует настроить вышеописанную логику частоты обновления в рамках их конфигурации._

1. Откройте **Home Assistant → Zigbee2MQTT**. Если Zigbee2MQTT отсутствует в боковом меню: **Настройки → Приложения → Zigbee2MQTT → Открыть веб-интерфейс**.
2. Перейдите в раздел **Устройства**.
3. Выберите нужную розетку.
4. Откройте вкладку **Настройки (специфичные)**.
5. Поменяйте "Measurement poll interval" на 1
