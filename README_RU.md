...
> Интеграция объединяет обычную лампу и умную розетку в одну сущность Home Assistant. Она определяет состояние и текущий режим яркости по потребляемому току, отображает режим в карточке и позволяет включить лампу, выключенную собственной кнопкой, кратко отключив и снова подав питание через розетку.
> 
