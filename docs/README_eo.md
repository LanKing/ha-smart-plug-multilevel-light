...
> Mia ingo ne aŭtomate raportas legadojn al HA kaj nur permesas balotadon je difinita intervalo. Defaŭlte ĝi estis 60 sekundoj. Necesas agordi ĉi tiun intervalon al 1 sekundo, alie vi atendos tre longan tempon por ke la reĝimŝanĝa ekrano aperos.
...
5. La ŝanĝoj **Mezura balotintervalo** je 1 s. <a id="installation"></a> ## 📦 Instalado ### 🛍 Instalado per HACS ℹ️ [Kio okazis HACS kaj kiel instali ĝin?](https://github.com/LanKing/ha-tools/blob/main/appendix-what-is-hacs/docs/README_eo.md) 🚀 [Provu aldoni deponejon ĉe la ligilo](https://my.home-assistant.io/redirect/hacs_repository/?owner=LanKing&repository=ha-smart-plug-multilevel-light&category=integration). Se via HA subtenas ĉi tiun metodon, alklaku **Aldoni** en la fenestro kiu malfermiĝas, kaj poste la butonon **Elŝutu** en la malsupra dekstra angulo. Se la instalado estis kompletigita sukcese, vi povas salti pliajn paŝojn. Saltu rekte al sekcio [rapida komenco](#getting-started). #### 1. Aldonante deponejon La deponejo ankoraŭ ne estas inkluzivita en la norma dosierujo HACS, aldonu ĝin kiel kutimo: 1. Malfermu **HACS → Integriĝoj**. 2. Malfermu la menuon en la supra dekstra angulo kaj elektu **Propraj Deponejoj**. 3. Aldoni: ```text https://github.com/LanKing/ha-smart-plug-multilevel-light ``` 4. Elektu tipon **Integriĝo** kaj premu **Aldoni**. > Aldono de deponejo nur ebligas la integriĝon en HACS. Por instali, vi devas aparte malfermi ĝian karton kaj klaki **Elŝutu**. #### 2. Instalado de integriĝo 1. Trovu en HACS: Smart Plug Multi-Level Light. 2. Malfermu la integriĝon kaj alklaku **Elŝutu**. 3. Post instalado, rekomencu tute Home Assistant. <a id="manual-installation"></a> ### 🧑‍💻 Instalado sen HACS 1. Sur la deponeja paĝo, alklaku **Kodo → Elŝutu ZIP**. 2. Malpaku la arkivon. 3. Kopiu la dosierujon: ```text custom_components/smart_plug_multilevel_light ``` al la agorda dosierujo Home Assistant: ```text /config/custom_components/smart_plug_multilevel_light ``` Fina dosiera vojo `manifest.json` devus aspekti jene: ```text /config/custom_components/smart_plug_multilevel_light/manifest.json ``` 4. Tute rekomencu Home Assistant. > Ne kopiu la tutan eksteran deponejan dosierujon al `custom_components`. Interne `custom_components` la dosierujo devas troviĝi rekte `smart_plug_multilevel_light`. La karto estas parto de la integriĝo, do HACS ĝi ne estas postulata por instalado. Post mane kopiado de dosieroj, rekomencu Home Assistant kaj kreante la unuan helpan objekton, la integriĝo publikigas la JavaScript-kartdosieron kaj aŭtomate registras ĝin kun la panelaj rimedoj en norma reĝimo `storage`. En YAML-rimeda reĝimo, la karto devas esti registrita permane, kiel priskribite malsupre. <a id="getting-started"></a> ## 🚀 Rapida komenco post instalado ### 🛠 Agordo 1. Malfermu **[Agordoj → Aparatoj kaj servoj → Akcesoraĵoj](https://my.home-assistant.io/redirect/helpers/)** kaj premu **Krei Helpan Objekton**. 2. Elektu **Smart Plug Multi-Level Light**. 3. Elektu la inteligentan ŝtopilon al kiu la lampo estas konektita. La listo montras nur ingojn kiuj havas Home Assistant vidas la potencosensilon en la sama aparato. 4. En la sekva paŝo, agordu la lampajn parametrojn: * **Lumnomo (Light name)** — la nomo de la kreata ento, ekzemple `FloorLamp`; * **Power sensor** - inga potenco-sensilo; 5. En la sekcio **🔅 Helecaj reĝimoj (🔅 Brightness modes)** aldonu almenaŭ unu brilreĝimon per klako **Aldoni**.<br /><img src="https://raw.githubusercontent.com/LanKing/ha-smart-plug-multilevel-light/main/docs/modes-edit.png" width="465" /><br />Ŝaltu la lampon per ĝia fizika butono, enigu la nomon de la reĝimo kaj komencu **Test stable power**. Post akiri stabilan valoron, premu **Apliki**.<br />&nbsp;
6. Post aldoni la unuan reĝimon, aldonu la ceterajn modojn ripetante ĉi tiujn paŝojn. Vi devus fini kun io kiel:<br /><img src="https://raw.githubusercontent.com/LanKing/ha-smart-plug-multilevel-light/main/docs/modes-list.png" width="457" /><br />&nbsp;<br />
...
Malfermu la panelon kie troviĝas la karto, iru al redakti reĝimo kaj alklaku **Redakti** sur la dezirata karto.

Malfermante la sekcion **Enhavo (Content)**, vi vidos la disponeblajn agordojn:

* **Montri reĝimnomon (Show mode name)** — montras aŭ kaŝas la nomon de la nuna reĝimo: ekzemple Malklara, Malalta, Meza aŭ Alta.
* **Montri procenton (Show percentage)** — montras aŭ kaŝas la kalkulitan brilecon kiel procento.

Malfermante la sekcion **Interagoj (Interactions)**, vi vidos:

* **Ĉiam montri ikonfonon (Always show icon background)** — konservas la rondan fonon ĉirkaŭ la ikono, eĉ se aparta ago por ĝi estas malŝaltita.

> 🚫 Cetere, se vi, kiel mi, uzas nenorman kart-ikonon kaj kiam ĝi estas malŝaltita ne estas vide forstrekita, mi rekomendas mian alian kromprogramon - [Mdi Off Fallback](https://github.com/LanKing/ha-mdi-off-fallback/blob/main/docs/README_eo.md), kiu solvas ĉi tiun estetikan problemon.
...
Rondiĝo nur influas la montritan procenton kaj vidan intensecon de la karto. La reĝimdifino kaj funkciado de la lampo ne ŝanĝiĝas. Defaŭlte, rondigo estas malŝaltita.

## 🧩 Lovelace-karto detale
...
### Lovelace rimedo YAML-reĝimo

> Plej verŝajne, vi ne havas ĉi tiun reĝimon ebligita. Se ne estas problemoj kaj la karto funkcias, saltu ĉi tiun paŝon.

Se vi havas `configuration.yaml` eksplicite fiksita:
...
Kontrolu **[Agordoj → Aparatoj kaj servoj → Objektoj](https://my.home-assistant.io/redirect/entities/)**:

* Ĉu ili apartenas `switch` kaj potenca sensilo por unu aparato?
* Ĉu la potencsensilo estas ŝaltita?
* Ĉu ĝi estas ĝuste precizigita? `device_class`?
* Ĉu ĝi havas nombran staton?
...
La bezonata nombro da legaĵoj estas agordita de la parametro **Consecutive readings to switch** kaj defaŭlte al `5`. Periodaj mezuradoj estas faritaj unufoje je sekundo, kaj ŝanĝoj en la sensilstato tuj estas procesitaj. Se dum testado la valoro respondas al malsama reĝimo, la retronombrado komenciĝas denove.

Ĝis konfirmo, la integriĝo konservas la antaŭan reĝimon. Danke al ĉi tio, la karto ne ŝanĝas inter modoj pro mezaj valoroj, kiuj okazas dum ŝanĝado de la lampo, stabiligo de ĝia potenco aŭ malgrandaj fluktuoj en sensillegaĵoj.

Redukti la nombron da legaĵoj plirapidigas la montradon de la nova reĝimo, sed igas la detekton pli sentema al potencfluktuoj.
...
En YAML-reĝimo [aldoni rimedon permane](#yaml-lovelace-mode).
...
3. Kontrolu **[Agordoj → Paneloj → ⋮ → Rimedoj](https://my.home-assistant.io/redirect/lovelace_resources/)** kaj mane forigu la kartan rimedon se ĝi restas:
...
Ĉi tiu projekto estas distribuita sub la permesilo [MIT](https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/LICENSE).
