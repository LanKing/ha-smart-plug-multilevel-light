...
* Crea una entitat regular Home Assistant `light`compatible amb automatitzacions, scripts i interfície HA estàndard;
...
* Durant la primera configuració, la potència es llegeix en temps real: l'usuari només ha de canviar la llum al següent mode i prémer el botó d'aprenentatge;
* Filtra amb èxit les lectures de potència inestables, evitant canvis caòtics de l'indicador de la targeta quan la potència "salta" dins d'un mode (i fins i tot coincideix amb el següent mode);
* Admet un nombre arbitrari de modes;
* La targeta està construïda a sobre de l'estàndard Home Assistant Tile Card conservant la seva geometria, fons, efectes de flotació, mides, tipografia i accions estàndard;
* A la targeta, la brillantor actual està codificada visualment per la intensitat del color;
* Calcula la brillantor linealment en relació a la potència màxima; es pot activar l'arrodoniment de la brillantor calculada al 5% per a una visualització més precisa dels percentatges;
* Permet personalitzar de manera flexible els paràmetres mostrats i l'aspecte de la targeta;
* Us permet seleccionar només el sensor de potència relacionat amb l'endoll intel·ligent seleccionat;
* Permet ajustar el retard del cicle de desactivació per al correcte funcionament amb diferents models de llums i endolls;
* Permet establir el llindar de potència per sota del qual el llum es considera apagat;
* Interfície localitzada per a 64 localitzacions Home Assistant.
...
> El meu sòcol no informa automàticament de les lectures a HA i només permet l'enquesta a un interval especificat. Per defecte era de 60 segons. Cal establir aquest interval en 1 segon, en cas contrari, esperareu molt de temps perquè aparegui la pantalla de canvi de mode.
...
1. Obert **Home Assistant → Zigbee2MQTT**. Si Zigbee2MQTT falta al menú lateral, obre **Configuració → Aplicacions → Zigbee2MQTT → Obre la interfície web**.
2. Vés a la secció **Dispositius**.
3. Seleccioneu la presa necessària.
4. Obriu la pestanya **Configuració (específica)**.
5. Els canvis **Interval d'enquesta de mesura** a 1 s. <a id="installation"></a> ## 📦 Instal·lació ### 🛍 Instal·lació via HACS ℹ️ [Què ha passat HACS i com instal·lar-lo?](https://github.com/LanKing/ha-tools/blob/main/appendix-what-is-hacs/docs/README_ca.md) 🚀 [Proveu d'afegir un repositori a l'enllaç](https://my.home-assistant.io/redirect/hacs_repository/?owner=LanKing&repository=ha-smart-plug-multilevel-light&category=integration). Si el vostre HA admet aquest mètode, feu clic **Afegeix** a la finestra que s'obre i després el botó **Descarregar** a la cantonada inferior dreta. Si la instal·lació s'ha completat correctament, podeu ometre passos addicionals. Saltar directament a la secció [inici ràpid](#getting-started). #### 1. Afegir un repositori El repositori encara no està inclòs al directori estàndard HACS, afegiu-lo com a personalitzat: 1. Obert **HACS → Integracions**. 2. Obriu el menú a l'extrem superior dret i seleccioneu **Repositoris personalitzats**. 3. Afegeix: ```text https://github.com/LanKing/ha-smart-plug-multilevel-light ``` 4. Seleccioneu el tipus **Integració** i premeu **Afegeix**. > Afegir un repositori només fa que la integració estigui disponible a HACS. Per instal·lar-lo, heu d'obrir la seva targeta per separat i fer clic **Descarregar**. #### 2. Instal·lació d'integració 1. Troba dins HACS: Smart Plug Multi-Level Light. 2. Obriu la integració i feu clic **Descarregar**. 3. Després de la instal·lació, reinicieu completament Home Assistant. <a id="manual-installation"></a> ### 🧑‍💻 Instal·lació sense HACS 1. A la pàgina del repositori, feu clic **Codi → Descarregar ZIP**. 2. Descomprimir l'arxiu. 3. Copia la carpeta: ```text custom_components/smart_plug_multilevel_light ``` al directori de configuració Home Assistant: ```text /config/custom_components/smart_plug_multilevel_light ``` Ruta del fitxer final `manifest.json` hauria de ser així: ```text /config/custom_components/smart_plug_multilevel_light/manifest.json ``` 4. Reinicieu completament Home Assistant. > No copieu tota la carpeta del repositori extern a `custom_components`. Dins `custom_components` la carpeta s'ha de localitzar directament `smart_plug_multilevel_light`. La targeta forma part de la integració, per tant HACS no és necessari per a la instal·lació. Després de copiar els fitxers manualment, reinicieu-lo Home Assistant i creant el primer objecte auxiliar, la integració publica el fitxer de la targeta JavaScript i el registra automàticament amb els recursos del panell en mode estàndard `storage`. En el mode de recursos YAML, la targeta s'ha de registrar manualment, tal com es descriu a continuació. <a id="getting-started"></a> ## 🚀 Inici ràpid després de la instal·lació ### 🛠 Configuració 1. Obert **[Configuració → Dispositius i serveis → Accessoris](https://my.home-assistant.io/redirect/helpers/)** i premeu **Crea un objecte d'ajuda**. 2. Seleccioneu **Smart Plug Multi-Level Light**. 3. Seleccioneu l'endoll intel·ligent al qual està connectada la làmpada. La llista només mostra els endolls que tenen Home Assistant veu el sensor de potència al mateix dispositiu. 4. Al següent pas, configureu els paràmetres de la làmpada: * **Nom de la llum (Light name)** — el nom de l'entitat que es crea, per exemple `FloorLamp`; * **Power sensor** - sensor de potència de presa; 5. A la secció **🔅 Modes de brillantor (🔅 Brightness modes)** afegeix almenys un mode de brillantor fent clic **Afegeix**.<br /><img src="https://raw.githubusercontent.com/LanKing/ha-smart-plug-multilevel-light/main/docs/modes-edit.png" width="465" /><br />Canvia el llum amb el seu botó físic, introdueix el nom del mode i comença **Test stable power**. Després d'obtenir un valor estable, premeu **Aplicar**.<br />&nbsp;
6. Després d'afegir el primer mode, afegiu els modes restants repetint aquests passos. Hauríeu d'acabar amb alguna cosa com:<br /><img src="https://raw.githubusercontent.com/LanKing/ha-smart-plug-multilevel-light/main/docs/modes-list.png" width="457" /><br />&nbsp;<br />
...
2. Seleccioneu **Smart Plug Multi-Level Light**, especifiqueu l'entitat creada `light` i guarda la targeta.
...
#### Brillantor arrodonit fins al 5%

Obert **[Configuració → Dispositius i serveis → Accessoris](https://my.home-assistant.io/redirect/helpers/)**, trobeu l'objecte auxiliar creat anteriorment **Smart Plug Multi-Level Light** i obriu la seva configuració. Encendre o apagar **Round brightness to 5% (may look nicer)**.

Quan la configuració està activada, el percentatge calculat s'arrodoneix al 5% més proper, per exemple: 33% → 35%, 67% → 65%.

L'arrodoniment només afecta el percentatge mostrat i la intensitat visual de la targeta. La definició del mode i el funcionament del llum no canvien. Per defecte, l'arrodoniment està desactivat.

## 🧩 Targeta Lovelace amb detall
...
### Mode YAML de recursos de Lovelace
...
Home Assistant no permet que la integració canviï automàticament la llista de recursos. En aquest cas, afegiu el recurs manualment:
...
Després del canvi, torneu a carregar els recursos de Lovelace o reinicieu-lo Home Assistant.
...
A continuació, feu un reinici dur Home Assistant i comproveu si hi ha errors al registre `smart_plug_multilevel_light`.
...
La reducció del nombre de lectures accelera la visualització del nou mode, però fa que la detecció sigui més sensible a les fluctuacions de potència.
...
2. Actualitzeu la pàgina per esborrar la memòria cau del vostre navegador.
3. Comproveu el número de versió a l'URL del recurs del panell.
4. A l'aplicació mòbil, tanqueu completament i torneu a obrir la interfície.
...
1. Suprimeix tots els objectes d'ajuda creats **Smart Plug Multi-Level Light** a través de **[Configuració → Dispositius i serveis → Accessoris](https://my.home-assistant.io/redirect/helpers/)**.
2. Elimina la integració via HACS o suprimiu manualment la carpeta:
...
Aquest projecte es distribueix sota la llicència [MIT](https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/LICENSE).

## 👋 El vostre suport
[Si aquest projecte us ha estalviat temps, podeu donar suport als meus experiments](https://github.com/sponsors/LanKing) (GitHub Sponsors). ☕🔧
