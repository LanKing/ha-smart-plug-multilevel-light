...
> ইন্টিগ্রেশনটি একটি অ-ডিজিটালি নিয়ন্ত্রিত লাইট ফিক্সচার এবং একটি হোম অ্যাসিস্ট্যান্ট সত্তার মধ্যে একটি স্মার্ট প্লাগকে একত্রিত করে। এটি পাওয়ার ব্যবহারের উপর ভিত্তি করে স্থিতি এবং বর্তমান উজ্জ্বলতা মোড নির্ধারণ করে, একটি কার্ডে সেগুলি প্রদর্শন করে এবং আপনাকে আউটলেটের মাধ্যমে সংক্ষিপ্তভাবে বন্ধ করে এবং পুনরায় প্রয়োগ করে নিজস্ব বোতাম দ্বারা বন্ধ করা একটি বাতি চালু করার অনুমতি দেয়।

# 🔌 স্মার্ট প্লাগ মাল্টি-লেভেল লাইট

![Пример режимов на карточке](ha-smart-plug-multilevel-light-card-modes-v4.png)

**✨ বৈশিষ্ট্য এবং বৈশিষ্ট্য:**

* একটি নেটিভ হোম সহকারী সত্তা তৈরি করে`light`অটোমেশন, স্ক্রিপ্ট এবং স্ট্যান্ডার্ড HA ইন্টারফেসের সাথে সামঞ্জস্যপূর্ণ;
...
* কার্ডটি স্ট্যান্ডার্ড হোম অ্যাসিস্ট্যান্ট টাইল কার্ডের উপরে তৈরি করা হয়েছে, এর জ্যামিতি, ব্যাকগ্রাউন্ড, হোভার ইফেক্ট, আকার, টাইপোগ্রাফি এবং স্ট্যান্ডার্ড অ্যাকশন সংরক্ষণ করে;
...
* ইন্টারফেসটি 64টি হোম অ্যাসিস্ট্যান্ট লোকেলের জন্য স্থানীয়করণ করা হয়েছে।
...
এই বিবরণ শুধুমাত্র Z2M এর মাধ্যমে সংযুক্ত সকেটের মালিকদের জন্য প্রাসঙ্গিক। অন্যান্য ব্যবহারকারীদের তাদের কনফিগারেশনের মধ্যে রিফ্রেশ হার সামঞ্জস্য করা উচিত:

1. খোলা**হোম অ্যাসিস্ট্যান্ট → Zigbee2MQTT**. Zigbee2MQTT পাশের মেনুতে না থাকলে, খুলুন**সেটিংস → অ্যাপ্লিকেশন → Zigbee2MQTT → ওয়েব ইন্টারফেস খুলুন**.
2. বিভাগে যান**ডিভাইস**.
...
ℹ️[HACS কি এবং কিভাবে ইন্সটল করতে হয়?](https://github.com/LanKing/ha-tools/blob/main/appendix-what-is-hacs/docs/README_bn.md)
...
যদিও সংগ্রহস্থলটি স্ট্যান্ডার্ড HACS ডিরেক্টরিতে অন্তর্ভুক্ত করা হয় না, এটি একটি কাস্টম হিসাবে যুক্ত করুন:
...
> একটি সংগ্রহস্থল যোগ করা শুধুমাত্র HACS এ একীকরণ উপলব্ধ করে। ইনস্টল করার জন্য, আপনাকে আলাদাভাবে এর কার্ড খুলতে হবে এবং ক্লিক করতে হবে**ডাউনলোড করুন**.

#### 2. ইন্টিগ্রেশন ইনস্টলেশন

1. HACS এ অনুসন্ধান করুন:`Smart Plug Multi-Level Light`.
2. ইন্টিগ্রেশন খুলুন এবং ক্লিক করুন**ডাউনলোড করুন**.
3. ইনস্টলেশনের পরে, হোম অ্যাসিস্ট্যান্ট সম্পূর্ণরূপে পুনরায় চালু করুন।

<a id="manual-installation"></a>

### 🧑‍💻 HACS ছাড়া ইনস্টলেশন
...
হোম সহকারী কনফিগারেশন ডিরেক্টরিতে:
...
4. হোম অ্যাসিস্ট্যান্ট সম্পূর্ণভাবে রিস্টার্ট করুন।
...
কার্ডটি ইন্টিগ্রেশনে অন্তর্ভুক্ত করা হয়েছে, তাই এটি ইনস্টল করার জন্য HACS এর প্রয়োজন নেই। ফাইলগুলি ম্যানুয়ালি কপি করার পরে, হোম অ্যাসিস্ট্যান্ট রিস্টার্ট করার পরে এবং প্রথম হেল্পার অবজেক্ট তৈরি করার পরে, ইন্টিগ্রেশন জাভাস্ক্রিপ্ট কার্ড ফাইলটি প্রকাশ করে এবং স্ট্যান্ডার্ড মোডে প্যানেল সংস্থানগুলির সাথে স্বয়ংক্রিয়ভাবে নিবন্ধিত হয়`storage`. YAML রিসোর্স মোডে, কার্ডটি অবশ্যই ম্যানুয়ালি নিবন্ধিত হতে হবে, যেমন নীচে বর্ণনা করা হয়েছে।
...
2. নির্বাচন করুন**স্মার্ট প্লাগ মাল্টি-লেভেল লাইট**.

3. ল্যাম্পটি যে স্মার্ট প্লাগটির সাথে সংযুক্ত তা নির্বাচন করুন। তালিকাটি শুধুমাত্র সেই আউটলেটগুলি দেখায় যেখানে হোম অ্যাসিস্ট্যান্ট একই ডিভাইসে পাওয়ার সেন্সর দেখতে পায়।
...
5. বিভাগে**ZUI003Z**ক্লিক করে অন্তত একটি উজ্জ্বলতা মোড যোগ করুন**যোগ করুন**.<br /> <img src="modes-edit.png" width="50%"/><br />ল্যাম্পটিকে এর ফিজিক্যাল বোতাম দিয়ে স্যুইচ করুন, মোডের নাম লিখুন এবং শুরু করুন**Test stable power**. একটি স্থিতিশীল মান পাওয়ার পরে, টিপুন**আবেদন করুন**.
...
7. সহায়ক বস্তু সংরক্ষণ করুন. হোম অ্যাসিস্ট্যান্ট একটি নতুন সত্তা তৈরি করবে`light`, যেমন`light.FloorLamp`.

### 🧩 লাভলেস কার্ড

1. পছন্দসই প্যানেল খুলুন, সম্পাদনা মোড চালু করুন এবং ক্লিক করুন**একটি কার্ড যোগ করুন**.
2. নির্বাচন করুন**স্মার্ট প্লাগ মাল্টি-লেভেল লাইট**, তৈরি সত্তা নির্দিষ্ট করুন`light`এবং কার্ড সংরক্ষণ করুন।
...
খোল&#x9BE;**[সেটিংস → ডিভাইস এবং পরিষেবা → আনুষাঙ্গিক](https://my.home-assistant.io/redirect/helpers/)**, আগে তৈরি সহায়ক বস্তু খুঁজুন**স্মার্ট প্লাগ মাল্টি-লেভেল লাইট**এবং সেটিংস খুলুন। চালু বা বন্ধ করুন**Round brightness to 5% (may look nicer)**.
...
হোম অ্যাসিস্ট্যান্ট ইন্টিগ্রেশনকে স্বয়ংক্রিয়ভাবে সম্পদের তালিকা পরিবর্তন করার অনুমতি দেয় না। এই ক্ষেত্রে, ম্যানুয়ালি সম্পদ যোগ করুন:
...
পরিবর্তনের পরে, লাভলেস রিসোর্স রিবুট করুন বা হোম অ্যাসিস্ট্যান্ট রিস্টার্ট করুন।
...
**সম্ভাব্য কারণ:**&#x9AD;ুল পথ বা হোম সহকারী পুনরায় চালু করা হয়নি।
...
তারপরে হোম অ্যাসিস্ট্যান্টের সম্পূর্ণ পুনরায় চালু করুন এবং ত্রুটিগুলির জন্য লগটি পরীক্ষা করুন`smart_plug_multilevel_light`.
...
একটি আউটলেট নির্বাচন করার পরে, ইন্টিগ্রেশন একটি পাওয়ার সেন্সরের উপস্থিতির জন্য পুনরায় পরীক্ষা করে। যদি সেন্সরটি সরানো হয়, অক্ষম করা হয়, অন্য ডিভাইসে সরানো হয়, বা থাকা বন্ধ হয়ে যায় তাহলে ত্রুটি ঘটে`device_class: power`.
...
কমপক্ষে একটি কনফিগার করা হেল্পার অবজেক্ট লোড করার পরে এবং শুধুমাত্র রিসোর্স মোডে স্বয়ংক্রিয় নিবন্ধন ঘটে`storage`. ইন্টিগ্রেশন ইনস্টল করার পদ্ধতি - HACS এর মাধ্যমে বা ম্যানুয়ালি - এটিকে প্রভাবিত করে না।
...
1. হোম অ্যাসিস্ট্যান্ট সম্পূর্ণভাবে রিস্টার্ট করুন।
...
1. সমস্ত তৈরি সহায়ক বস্তু মুছুন**স্মার্ট প্লাগ মাল্টি-লেভেল লাইট**মাধ্যম&#x9C7;**[সেটিংস → ডিভাইস এবং পরিষেবা → আনুষাঙ্গিক](https://my.home-assistant.io/redirect/helpers/)**.
2. HACS এর মাধ্যমে ইন্টিগ্রেশন সরান বা ম্যানুয়ালি ফোল্ডারটি মুছুন:
...
প্রকল্প লাইসেন্সের অধীনে বিতরণ করা হয়[সঙ্গে](../LICENSE).

<br />

> \[!টিপ]**আপনি সমাধান পছন্দ করেন?**&#x9A6;েখুন 🏠<a href="https://github.com/LanKing/ha-tools/blob/main/docs/README_bn.md">হোম সহকারীর জন্য আমার অন্যান্য পণ্য</a>
