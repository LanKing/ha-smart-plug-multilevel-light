...
ℹ️ [Wat het gebeur HACS en hoe om dit te installeer?](https://github.com/LanKing/ha-tools/blob/main/appendix-what-is-hacs/docs/README_af.md)
...
Die bewaarplek is nog nie in die standaardgids ingesluit nie HACS, voeg dit by soos pasgemaak:
...
> Die byvoeging van 'n bewaarplek maak die integrasie slegs beskikbaar in HACS. Om te installeer, moet jy die kaart apart oopmaak en klik **Laai af**.

#### 2. Installering van integrasie

1. Vind in HACS: Smart Plug Multi-Level Light.
2. Maak die integrasie oop en klik **Laai af**.
3. Na die installasie, herbegin heeltemal Home Assistant.
...
na die konfigurasiegids Home Assistant:

```text
/config/custom_components/smart_plug_multilevel_light
```

Finale lêerpad `manifest.json` moet so lyk:
...
> Moenie die hele eksterne bewaarpleklêergids na kopieer nie `custom_components`. Binne `custom_components` die gids moet direk geleë wees `smart_plug_multilevel_light`.

Die kaart is deel van die integrasie, dus HACS dit is nie nodig vir installasie nie. Nadat u lêers handmatig gekopieer het, herbegin Home Assistant en die skep van die eerste hulpobjek, die integrasie publiseer die JavaScript-kaartlêer en registreer dit outomaties met die paneelhulpbronne in standaardmodus `storage`. In YAML-hulpbronmodus moet die kaart met die hand geregistreer word, soos hieronder beskryf.

<a id="getting-started"></a>

## 🚀 Vinnige begin na installasie
...
2. Kies **Smart Plug Multi-Level Light**.

3. Kies die slimprop waaraan die lamp gekoppel is. Die lys wys slegs voetstukke wat het Home Assistant sien die kragsensor in dieselfde toestel.
