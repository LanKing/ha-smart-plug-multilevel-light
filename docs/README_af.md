...
* Skep 'n gereelde entiteit Home Assistant `light`versoenbaar is met outomatisering, skrifte en standaard HA-koppelvlak;
...
* Tydens die eerste instelling word die krag intyds gelees: die gebruiker hoef net die lig na die volgende modus oor te skakel en die leerknoppie te druk;
* Filtreer onstabiele kraglesings suksesvol uit, wat chaotiese veranderinge van die aanwyser op die kaart voorkom wanneer die krag binne een modus "spring" (en selfs saamval met die volgende modus);
* Ondersteun 'n arbitrêre aantal modusse;
* Die kaart is bo-op die standaard een gebou Home Assistant Teëlkaart wat sy geometrie, agtergrond, sweefeffekte, groottes, tipografie en standaardaksies behou;
* In die kaart word die huidige helderheid visueel geënkodeer deur kleurintensiteit;
* Bereken helderheid lineêr relatief tot maksimum krag; afronding van die berekende helderheid tot 5% kan geaktiveer word vir 'n meer akkurate vertoning van persentasies;
* Laat jou toe om die vertoonde parameters en voorkoms van die kaart buigsaam aan te pas;
* Laat jou toe om slegs die kragsensor te kies wat verband hou met die gekose slimprop;
* Laat jou toe om die vertraging van die ontspanningsiklus aan te pas vir korrekte werking met verskillende modelle van lampe en voetstukke;
* Laat jou toe om die kragdrempel te stel waaronder die lamp as af beskou word;
* Koppelvlak gelokaliseer vir 64 plekke Home Assistant.
...
> My sok rapporteer nie outomaties lesings aan HA nie en laat slegs polling met 'n gespesifiseerde interval toe. By verstek was dit 60 sekondes. Dit is nodig om hierdie interval op 1 sekonde in te stel, anders sal jy baie lank wag vir die modusskakelskerm om te verskyn.

Hierdie beskrywing is slegs relevant vir eienaars van voetstukke wat via verbind is Z2M. Ander gebruikers moet die herlaaitempo binne hul konfigurasie aanpas:

1. Maak oop **Home Assistant → Zigbee2MQTT**. As Zigbee2MQTT ontbreek in die sykieslys, maak oop **Instellings → Toepassings → Zigbee2MQTT → Maak webkoppelvlak oop**.
2. Gaan na afdeling **Toestelle**.
3. Kies die verlangde uitlaat.
4. Maak die oortjie oop **Instellings (spesifiek)**.
5. Die veranderinge **Metingspeilingsinterval** op 1 s. <a id="installation"></a> ## 📦 Installasie ### 🛍 Installasie via HACS ℹ️ [Wat het gebeur HACS en hoe om dit te installeer?](https://github.com/LanKing/ha-tools/blob/main/appendix-what-is-hacs/docs/README_af.md) 🚀 [Probeer om 'n bewaarplek by die skakel by te voeg](https://my.home-assistant.io/redirect/hacs_repository/?owner=LanKing&repository=ha-smart-plug-multilevel-light&category=integration). As jou HA hierdie metode ondersteun, klik **Voeg by** in die venster wat oopmaak, en dan die knoppie **Laai af** in die onderste regterhoek. As die installasie suksesvol voltooi is, kan jy verdere stappe oorslaan. Slaan reguit na afdeling [vinnige begin](#getting-started). #### 1. Voeg 'n bewaarplek by Die bewaarplek is nog nie in die standaardgids ingesluit nie HACS, voeg dit by soos pasgemaak: 1. Maak oop **HACS → Integrasies**. 2. Maak die kieslys in die regter boonste hoek oop en kies **Pasgemaakte bewaarplekke**. 3. Voeg by: ```text https://github.com/LanKing/ha-smart-plug-multilevel-light ``` 4. Kies tipe **Integrasie** en druk **Voeg by**. > Die byvoeging van 'n bewaarplek maak die integrasie slegs beskikbaar in HACS. Om te installeer, moet jy die kaart apart oopmaak en klik **Laai af**. #### 2. Installering van integrasie 1. Vind in HACS: Smart Plug Multi-Level Light. 2. Maak die integrasie oop en klik **Laai af**. 3. Na die installasie, herbegin heeltemal Home Assistant. <a id="manual-installation"></a> ### 🧑‍💻 Installasie sonder HACS 1. Klik op die bewaarplekbladsy **Kode → Laai zip af**. 2. Pak die argief uit. 3. Kopieer die gids: ```text custom_components/smart_plug_multilevel_light ``` na die konfigurasiegids Home Assistant: ```text /config/custom_components/smart_plug_multilevel_light ``` Finale lêerpad `manifest.json` moet so lyk: ```text /config/custom_components/smart_plug_multilevel_light/manifest.json ``` 4. Herbegin heeltemal Home Assistant. > Moenie die hele eksterne bewaarpleklêergids na kopieer nie `custom_components`. Binne `custom_components` die gids moet direk geleë wees `smart_plug_multilevel_light`. Die kaart is deel van die integrasie, dus HACS dit is nie nodig vir installasie nie. Nadat u lêers handmatig gekopieer het, herbegin Home Assistant en die skep van die eerste hulpobjek, die integrasie publiseer die JavaScript-kaartlêer en registreer dit outomaties met die paneelhulpbronne in standaardmodus `storage`. In YAML-hulpbronmodus moet die kaart met die hand geregistreer word, soos hieronder beskryf. <a id="getting-started"></a> ## 🚀 Vinnige begin na installasie ### 🛠 Konfigurasie 1. Maak oop **[Instellings → Toestelle en dienste → Bykomstighede](https://my.home-assistant.io/redirect/helpers/)** en druk **Skep Helper Object**. 2. Kies **Smart Plug Multi-Level Light**. 3. Kies die slimprop waaraan die lamp gekoppel is. Die lys wys slegs voetstukke wat het Home Assistant sien die kragsensor in dieselfde toestel. 4. Stel in die volgende stap die lampparameters in: * **Lignaam (Light name)** — die naam van die entiteit wat geskep word, byvoorbeeld `FloorLamp`; * **Power sensor** - sok krag sensor; 5. In die afdeling **🔅 Helderheidsmodusse (🔅 Brightness modes)** voeg ten minste een helderheidsmodus by deur te klik **Voeg by**.<br /><img src="https://raw.githubusercontent.com/LanKing/ha-smart-plug-multilevel-light/main/docs/modes-edit.png" width="465" /><br />Skakel die lamp met sy fisiese knoppie, voer die naam van die modus in en begin **Test stable power**. Nadat u 'n stabiele waarde verkry het, druk **Doen aansoek**.<br />&nbsp;
6. Nadat u die eerste modus bygevoeg het, voeg die oorblywende modusse by deur hierdie stappe te herhaal. Jy moet eindig met iets soos:<br /><img src="https://raw.githubusercontent.com/LanKing/ha-smart-plug-multilevel-light/main/docs/modes-list.png" width="457" /><br />&nbsp;<br />
...
2. Kies **Smart Plug Multi-Level Light**, spesifiseer die geskepte entiteit `light` en stoor die kaart.

As die kaart nie een van die beskikbare is nie, gebruik jy dalk [Lovelace hulpbron YAML-modus](#yaml-lovelace-mode).
...
Maak oop **[Instellings → Toestelle en dienste → Bykomstighede](https://my.home-assistant.io/redirect/helpers/)**, vind die helper-objek wat vroeër geskep is **Smart Plug Multi-Level Light** en maak sy instellings oop. Skakel aan of af **Round brightness to 5% (may look nicer)**.

Wanneer die instelling geaktiveer is, word die berekende persentasie tot die naaste 5% afgerond, byvoorbeeld: 33% → 35%, 67% → 65%.

Afronding beïnvloed slegs die vertoonde persentasie en visuele intensiteit van die kaart. Die modusdefinisie en werking van die lamp verander nie. By verstek is afronding gedeaktiveer.
...
Home Assistant laat nie toe dat die integrasie outomaties die lys hulpbronne verander nie. Voeg in hierdie geval die hulpbron handmatig by:
...
Na die verandering, herlaai Lovelace-hulpbronne of herbegin Home Assistant.
...
Doen dan 'n harde herbegin Home Assistant en kyk na die log vir foute `smart_plug_multilevel_light`.
...
Die vermindering van die aantal lesings versnel die vertoning van die nuwe modus, maar maak die opsporing meer sensitief vir kragskommelings.
...
2. Herlaai die bladsy om jou blaaier se kas skoon te maak.
3. Gaan die weergawenommer in die paneelhulpbron-URL na.
4. In die mobiele toepassing, maak die koppelvlak heeltemal toe en heropen.
...
1. Vee alle geskepte helper-voorwerpe uit **Smart Plug Multi-Level Light** deur **[Instellings → Toestelle en dienste → Bykomstighede](https://my.home-assistant.io/redirect/helpers/)**.
...
4. Herbegin heeltemal Home Assistant.
5. Indien nodig, verfris die bladsy om die koppelvlakkas skoon te maak.

## 📄 Lisensie

Hierdie projek word onder die [MIT](https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/LICENSE)-lisensie versprei.
