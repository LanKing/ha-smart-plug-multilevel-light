<sub>🇺🇸&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/README.md">E⁠n⁠g⁠l⁠i⁠s⁠h</a>&nbsp;|</sub> <sub>🇺🇦&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_uk.md">У⁠к⁠р⁠а⁠ї⁠н⁠с⁠ь⁠к⁠а</a>&nbsp;|</sub> <sub>ru&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ru.md">Р⁠у⁠с⁠с⁠к⁠и⁠й</a>&nbsp;|</sub> <sub>🇿🇦&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_af.md">A⁠f⁠r⁠i⁠k⁠a⁠a⁠n⁠s</a>&nbsp;|</sub> <sub>🇸🇦&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ar.md">ا⁠ل⁠ع⁠ر⁠ب⁠ي⁠ة</a>&nbsp;|</sub> <sub>🇧🇩&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_bn.md">বাং⁠লা</a>&nbsp;|</sub> <sub>🇧🇦&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_bs.md">B⁠o⁠s⁠a⁠n⁠s⁠k⁠i</a>&nbsp;|</sub> <sub>ca&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ca.md">C⁠a⁠t⁠a⁠l⁠à</a>&nbsp;|</sub> <sub>🇨🇿&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_cs.md">Č⁠e⁠š⁠t⁠i⁠n⁠a</a>&nbsp;|</sub> <sub>cy&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_cy.md">C⁠y⁠m⁠r⁠a⁠e⁠g</a>&nbsp;|</sub> <sub>🇩🇰&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_da.md">D⁠a⁠n⁠s⁠k</a>&nbsp;|</sub> <sub>🇩🇪&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_de.md">D⁠e⁠u⁠t⁠s⁠c⁠h</a>&nbsp;|</sub> <sub>🇬🇷&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_el.md">Ε⁠λ⁠λ⁠η⁠ν⁠ι⁠κ⁠ά</a>&nbsp;|</sub> <sub>eo&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_eo.md">E⁠s⁠p⁠e⁠r⁠a⁠n⁠t⁠o</a>&nbsp;|</sub> <sub>🇪🇸&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_es.md">E⁠s⁠p⁠a⁠ñ⁠o⁠l</a>&nbsp;|</sub> <sub>🇪🇪&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_et.md">E⁠e⁠s⁠t⁠i</a>&nbsp;|</sub> <sub>eu&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_eu.md">E⁠u⁠s⁠k⁠a⁠r⁠a</a>&nbsp;|</sub> <sub>🇮🇷&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_fa.md">ف⁠ا⁠ر⁠س⁠ی</a>&nbsp;|</sub> <sub>🇫🇮&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_fi.md">S⁠u⁠o⁠m⁠i</a>&nbsp;|</sub> <sub>fy&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_fy.md">F⁠r⁠y⁠s⁠k</a>&nbsp;|</sub> <sub>🇫🇷&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_fr.md">F⁠r⁠a⁠n⁠ç⁠a⁠i⁠s</a>&nbsp;|</sub> <sub>🇮🇪&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ga.md">G⁠a⁠e⁠i⁠l⁠g⁠e</a>&nbsp;|</sub> <sub>gl&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_gl.md">G⁠a⁠l⁠e⁠g⁠o</a>&nbsp;|</sub> <sub>🇨🇭&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_gsw.md">S⁠c⁠h⁠w⁠i⁠i⁠z⁠e⁠r⁠d⁠ü⁠t⁠s⁠c⁠h</a>&nbsp;|</sub> <sub>🇮🇱&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_he.md">ע⁠ב⁠ר⁠י⁠ת</a>&nbsp;|</sub> <sub>🇮🇳&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_hi.md">हि⁠न्दी</a>&nbsp;|</sub> <sub>🇭🇷&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_hr.md">H⁠r⁠v⁠a⁠t⁠s⁠k⁠i</a>&nbsp;|</sub> <sub>🇭🇺&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_hu.md">M⁠a⁠g⁠y⁠a⁠r</a>&nbsp;|</sub> <sub>🇦🇲&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_hy.md">Հ⁠ա⁠յ⁠ե⁠ր⁠ե⁠ն</a>&nbsp;|</sub> <sub>🇮🇩&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_id.md">I⁠n⁠d⁠o⁠n⁠e⁠s⁠i⁠a</a>&nbsp;|</sub> <sub>🇮🇸&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_is.md">Í⁠s⁠l⁠e⁠n⁠s⁠k⁠a</a>&nbsp;|</sub> <sub>🇮🇹&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_it.md">I⁠t⁠a⁠l⁠i⁠a⁠n⁠o</a>&nbsp;|</sub> <sub>🇯🇵&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ja.md">日⁠本⁠語</a>&nbsp;|</sub> <sub>🇬🇪&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ka.md">K⁠a⁠r⁠t⁠u⁠l⁠i</a>&nbsp;|</sub> <sub>🇰🇷&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ko.md">한⁠국⁠어</a>&nbsp;|</sub> <sub>🇱🇺&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_lb.md">L⁠ë⁠t⁠z⁠e⁠b⁠u⁠e⁠r⁠g⁠e⁠s⁠c⁠h</a>&nbsp;|</sub> <sub>🇱🇹&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_lt.md">L⁠i⁠e⁠t⁠u⁠v⁠i⁠ų</a>&nbsp;|</sub> <sub>🇱🇻&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_lv.md">L⁠a⁠t⁠v⁠i⁠e⁠š⁠u</a>&nbsp;|</sub> <sub>🇲🇰&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_mk.md">М⁠а⁠к⁠е⁠д⁠о⁠н⁠с⁠к⁠и</a>&nbsp;|</sub> <sub>🇮🇳&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ml.md">മ⁠ല⁠യാ⁠ളം</a>&nbsp;|</sub> <sub>🇳🇴&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_nb.md">N⁠o⁠r⁠s⁠k⁠&nbsp;⁠B⁠o⁠k⁠m⁠å⁠l</a>&nbsp;|</sub> <sub>🇳🇱&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_nl.md">N⁠e⁠d⁠e⁠r⁠l⁠a⁠n⁠d⁠s</a>&nbsp;|</sub> <sub>🇳🇴&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_nn.md">N⁠o⁠r⁠s⁠k⁠&nbsp;⁠N⁠y⁠n⁠o⁠r⁠s⁠k</a>&nbsp;|</sub> <sub>🇵🇱&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_pl.md">P⁠o⁠l⁠s⁠k⁠i</a>&nbsp;|</sub> <sub>🇵🇹&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_pt.md">P⁠o⁠r⁠t⁠u⁠g⁠u⁠ê⁠s</a>&nbsp;|</sub> <sub>🇧🇷&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_pt-BR.md">P⁠o⁠r⁠t⁠u⁠g⁠u⁠ê⁠s⁠&nbsp;⁠(⁠B⁠R⁠)</a>&nbsp;|</sub> <sub>🇷🇴&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ro.md">R⁠o⁠m⁠â⁠n⁠ă</a>&nbsp;|</sub> <sub>🇸🇰&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_sk.md">S⁠l⁠o⁠v⁠e⁠n⁠č⁠i⁠n⁠a</a>&nbsp;|</sub> <sub>🇸🇮&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_sl.md">S⁠l⁠o⁠v⁠e⁠n⁠š⁠č⁠i⁠n⁠a</a>&nbsp;|</sub> <sub>🇦🇱&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_sq.md">S⁠h⁠q⁠i⁠p</a>&nbsp;|</sub> <sub>🇷🇸&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_sr.md">С⁠р⁠п⁠с⁠к⁠и</a>&nbsp;|</sub> <sub>🇷🇸&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_sr-Latn.md">S⁠r⁠p⁠s⁠k⁠i</a>&nbsp;|</sub> <sub>🇸🇪&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_sv.md">S⁠v⁠e⁠n⁠s⁠k⁠a</a>&nbsp;|</sub> <sub>🇮🇳&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ta.md">த⁠மி⁠ழ்</a>&nbsp;|</sub> <sub>🇮🇳&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_te.md">తె⁠లు⁠గు</a>&nbsp;|</sub> <sub>🇹🇭&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_th.md">ภ⁠า⁠ษ⁠า⁠ไ⁠ท⁠ย</a>&nbsp;|</sub> <sub>🇹🇷&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_tr.md">T⁠ü⁠r⁠k⁠ç⁠e</a>&nbsp;|</sub> <sub>🇵🇰&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_ur.md">اُ⁠ر⁠دُ⁠و</a>&nbsp;|</sub> <sub>🇻🇳&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_vi.md">T⁠i⁠ế⁠n⁠g⁠&nbsp;⁠V⁠i⁠ệ⁠t</a>&nbsp;|</sub> <sub>🇨🇳&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_zh-Hans.md">简⁠体⁠中⁠文</a>&nbsp;|</sub> <sub>🇹🇼&nbsp;<a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/docs/README_zh-Hant.md">繁⁠體⁠中⁠文</a></sub>

<!-- markdown-translator:f30c10bb91a0f6587cd22ee0c70aa5e504b1807979a245dd1c91d57aadca6019 -->
<a href="https://www.home-assistant.io/"><img src="https://img.shields.io/badge/Home%20Assistant-Helper-41BDF5?logo=homeassistant&logoColor=white" alt="Home Assistant"></a> <a href="https://hacs.xyz/"><img src="https://img.shields.io/badge/HACS-Integration-41BDF5" alt="HACS"></a> <a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/releases"><img src="https://img.shields.io/github/v/release/LanKing/ha-smart-plug-multilevel-light?label=release&cacheSeconds=300" alt="Latest release"></a> <a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/releases"><img src="https://img.shields.io/github/downloads/LanKing/ha-smart-plug-multilevel-light/total?label=downloads&color=95991a&cacheSeconds=300" alt="Downloads"></a> <a href="https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/LICENSE"><img src="https://img.shields.io/github/license/LanKing/ha-smart-plug-multilevel-light?cacheSeconds=300" alt="License"></a> <a href="https://github.com/sponsors/LanKing"><img src="https://img.shields.io/badge/☕-%D0%9F%D0%BE%D0%B4%D0%BA%D1%80%D0%B5%D0%BF%D0%B5%D1%82%D0%B5%20%D1%80%D0%B0%D0%B1%D0%BE%D1%82%D0%B0%D1%82%D0%B0%20%D0%BC%D0%B8-ea4aaa" alt="Подкрепете работата ми"></a>

> Интеграцията съчетава нецифрова контролна лампа и интелигентен щепсел в едно цяло Home Assistant. Той определя състоянието и текущия режим на яркост въз основа на консумацията на енергия, показва ги в карта и ви позволява да включите лампа, изключена от собствен бутон, като изключите за кратко и подадете отново захранването през контакта.

# 🔌 Smart Plug Multi-Level Light

![Пример режимов на карточке](https://raw.githubusercontent.com/LanKing/ha-smart-plug-multilevel-light/main/docs/ha-smart-plug-multilevel-light-card-modes-v4.png)

**✨ Характеристики и функции:**

* Създава обикновен обект Home Assistant `light`съвместим с автоматизации, скриптове и стандартен HA интерфейс;
* Правилно включва лампата, независимо от това как е била изключена: ако захранването е било изключено през интелигентния контакт, той просто я включва; ако лампата е била изключена със собствен бутон, докато контактът е бил включен, тя извършва цикъл на изключване и повторно включване;
* Автоматично определя текущия режим на работа на лампата на база реално измерената стабилна мощност;
* Има прост конфигуратор, в който потребителят последователно назовава режимите, поддържани от неговата лампа, и системата се учи, като определя стабилната мощност във всеки режим;
* По време на първата настройка мощността се отчита в реално време: потребителят трябва само да превключи светлината на следващия режим и да натисне бутона за обучение;
* Успешно филтрира нестабилните показания на мощността, предотвратявайки хаотичните промени на индикатора на картата, когато мощността "скача" в рамките на един режим (и дори съвпада със следващия режим);
* Поддържа произволен брой режими;
* Картата е изградена върху стандартната Home Assistant Tile Card, запазвайки нейната геометрия, фон, ефекти при задържане, размери, типография и стандартни действия;
* В картата текущата яркост е визуално кодирана от интензитета на цвета;
* Изчислява яркостта линейно спрямо максималната мощност; може да се активира закръгляване на изчислената яркост до 5% за по-точно показване на проценти;
* Позволява гъвкаво да персонализирате показваните параметри и външния вид на картата;
* Позволява ви да изберете само сензора за мощност, свързан с избрания интелигентен щепсел;
* Позволява да регулирате забавянето на цикъла на изключване за правилна работа с различни модели лампи и контакти;
* Позволява ви да зададете прага на мощността, под който лампата се счита за изключена;
* Интерфейсът е локализиран за 64 езика Home Assistant.

## 🗜 Приготвяне на желязо

Ще ви трябва:

1. Интелигентен контакт, който трябва да осигурява контрол на мощността (превключвател) и измерване на мощността (сензор с device\_class: мощност) към HA. И двата обекта трябва да принадлежат към едно и също устройство и да са активирани.
2. Глупава лампа, която се включва автоматично при подаване на захранване и в идеалния случай има няколко режима на яркост.

> Използвах гнездо [Tuya TS011F (в калъф Girier)](https://www.zigbee2mqtt.io/devices/TS011F_plug_3.html). Ще кажа, че в началото на експериментите серията гнезда Girier не пасна поради неточни измервания, но този модел го направи, въпреки че беше и в случая Girier.

### 👨‍🔬 Важна настройка, която приложих към моя търговски обект:

> Моят сокет не отчита автоматично показанията на HA и позволява запитване само на определен интервал. По подразбиране беше 60 секунди. Необходимо е да зададете този интервал на 1 секунда, в противен случай ще чакате много дълго време, за да се появи дисплеят за превключване на режима.

Това описание е приложимо само за собствениците на гнезда, свързани чрез Z2M. Други потребители трябва да коригират честотата на опресняване в своята конфигурация:

1. Отворете **Home Assistant → Zigbee2MQTT**. Ако Zigbee2MQTT липсва в страничното меню, отворено **Настройки → Приложения → Zigbee2MQTT → Отворете уеб интерфейса**.
2. Отидете в раздел **Устройства**.
3. Изберете необходимия изход.
4. Отворете раздела **Настройки (специфични)**.
5. Промените **Интервал на анкета на измерване** на 1 с. <a id="installation"></a> ## 📦 Монтаж ### 🛍 Инсталиране чрез HACS ℹ️ [какво стана HACS и как да го инсталирам?](https://github.com/LanKing/ha-tools/blob/main/appendix-what-is-hacs/docs/README_bg.md) 🚀 [Опитайте да добавите хранилище на връзката](https://my.home-assistant.io/redirect/hacs_repository/?owner=LanKing&repository=ha-smart-plug-multilevel-light&category=integration). Ако вашата HA поддържа този метод, щракнете **Добавете** в прозореца, който се отваря, и след това бутона **Изтегляне** в долния десен ъгъл. Ако инсталацията е завършена успешно, можете да пропуснете следващите стъпки. Преминете направо към раздела [бърз старт](#getting-started). #### 1. Добавяне на хранилище Хранилището все още не е включено в стандартната директория HACS, добавете го като потребителски: 1. Отворете **HACS → Интеграции**. 2. Отворете менюто в горния десен ъгъл и изберете **Персонализирани хранилища**. 3. добавете: ```text https://github.com/LanKing/ha-smart-plug-multilevel-light ``` 4. Изберете тип **Интеграция** и натиснете **Добавете**. > Добавянето на хранилище прави интеграцията достъпна само в HACS. За да инсталирате, трябва отделно да отворите картата му и да щракнете **Изтегляне**. #### 2. Инсталиране на интеграция 1. Намерете в HACS: Smart Plug Multi-Level Light. 2. Отворете интеграцията и щракнете **Изтегляне**. 3. След инсталацията рестартирайте напълно Home Assistant. <a id="manual-installation"></a> ### 🧑‍💻 Монтаж без HACS 1. На странице репозитория нажмите **Код → Изтегляне на ZIP**. 2. Разопаковайте архива. 3. Копирайте папката: ```text custom_components/smart_plug_multilevel_light ``` в каталог конфигурации Home Assistant: ```text /config/custom_components/smart_plug_multilevel_light ``` Краен път на файла `manifest.json` трябва да изглежда така: ```text /config/custom_components/smart_plug_multilevel_light/manifest.json ``` 4. Напълно рестартирайте Home Assistant. > Не копирайте цялата папка на външното хранилище в `custom_components`. Вътре `custom_components` папката трябва да се намира директно `smart_plug_multilevel_light`. Картата е част от интеграцията, така че HACS не е необходимо за монтаж. След ръчно копиране на файлове, рестартиране Home Assistant и създаване на първия спомагателен обект, интеграцията публикува файла на картата на JavaScript и автоматично го регистрира с ресурсите на панела в стандартен режим `storage`. В режим на ресурс YAML картата трябва да се регистрира ръчно, както е описано по-долу. <a id="getting-started"></a> ## 🚀 Бърз старт след инсталиране ### 🛠 Конфигурация 1. Отворете **[Настройки → Устройства и услуги → Аксесоари](https://my.home-assistant.io/redirect/helpers/)** и натиснете **Създаване на помощен обект**. 2. Изберете **Smart Plug Multi-Level Light**. 3. Изберете интелигентния щепсел, към който е свързана лампата. Списъкът показва само гнезда, които имат Home Assistant вижда сензора за мощност в същото устройство. 4. В следващата стъпка задайте параметрите на лампата: * **Име на светлината (Light name)** — например името на създавания обект `FloorLamp`; * **Power sensor** - сензор за мощност на гнездото; 5. В секцията **🔅 Режими на яркост (🔅 Brightness modes)** добавете поне един режим на яркост, като щракнете **Добавете**.<br /><img src="https://raw.githubusercontent.com/LanKing/ha-smart-plug-multilevel-light/main/docs/modes-edit.png" width="465" /><br />Превключете лампата с нейния физически бутон, въведете името на режима и стартирайте **Test stable power**. След като получите стабилна стойност, натиснете **Кандидатствайте**.<br />&nbsp;
6. След като добавите първия режим, добавете останалите режими, като повторите тези стъпки. Трябва да завършите с нещо като:<br /><img src="https://raw.githubusercontent.com/LanKing/ha-smart-plug-multilevel-light/main/docs/modes-list.png" width="457" /><br />&nbsp;<br />
7. Запазете помощния обект. Home Assistant ще създаде нов обект `light`, например `light.FloorLamp`.

### 🧩 Lovelace картичка

1. Отворете желания панел, включете режима за редактиране и щракнете **Добавяне на карта**.
2. Изберете **Smart Plug Multi-Level Light**, посочете създадения обект `light` и запазете картата.

Ако картата не е сред наличните, може да използвате [Ресурс на Lovelace YAML режим](#yaml-lovelace-mode).

### 🎨 Визуални ефекти

#### Настройки в картата

Отворете панела, където се намира картата, отидете в режим на редактиране и щракнете **Редактиране** на желаната карта.

Отваряне на секцията **Съдържание (Content)**, ще видите наличните настройки:

* **Показвай името на режима (Show mode name)** — показва или скрива името на текущия режим: например Dim, Low, Medium или High.
* **Показвай процент (Show percentage)** — показва или скрива изчислената яркост като процент.

Отваряне на секцията **Взаимодействия (Interactions)**, ще видите:

* **Винаги показвай фона на иконата (Always show icon background)** — запазва кръглия фон около иконата, дори ако отделно действие за нея е деактивирано.

> 🚫 Между другото, ако вие като мен използвате нестандартна икона на карта и когато е изключена, тя не е визуално зачеркната, препоръчвам другия ми плъгин - [Mdi Off Fallback](https://github.com/LanKing/ha-mdi-off-fallback/blob/main/docs/README_bg.md), което решава този естетически проблем.

#### Закръгляване на яркостта до 5%

Отворете **[Настройки → Устройства и услуги → Аксесоари](https://my.home-assistant.io/redirect/helpers/)**, намерете създадения по-рано помощен обект **Smart Plug Multi-Level Light** и отворете настройките му. Включване или изключване **Round brightness to 5% (may look nicer)**.

Когато настройката е активирана, изчисленият процент се закръгля до най-близките 5%, например: 33% → 35%, 67% → 65%.

Закръгляването засяга само показания процент и визуалния интензитет на картата. Дефиницията на режима и работата на лампата не се променят. По подразбиране закръгляването е деактивирано.

## 🧩 Lovelace карта в детайли

Налични опции:

| Параметър | Тип | По подразбиране | Описание |
|---|---|---:|---|
| `entity` | string | изисква се | образувание, създадено от интеграцията |
| `name` | string | име на обект | заменя името, показано на картата |
| `icon` | string | икона на обект | заменя иконата |
| `show_mode` | boolean | `true` | показва името на режима |
| `show_percentage` | boolean | `true` | показва синтетичния процент |
| `icon_tap_action` | action | `more-info` | допълнително действие при докосване на иконата |
| `always_show_icon_background` | boolean | `false` | винаги показва фона на кръглата икона, дори когато None е избрано за icon_tap_action |

Пълен пример:

```yaml
type: custom:smart-plug-multilevel-light-card
entity: light.floor_lamp
name: Торшер
icon: mdi:floor-lamp
show_mode: true
show_percentage: true
icon_tap_action:
  action: more-info
always_show_icon_background: false
```

<a id="yaml-lovelace-mode"></a>

### Ресурс на Lovelace YAML режим

> Най-вероятно не сте активирали този режим. Ако няма проблеми и картата работи, пропуснете тази стъпка.

Ако имате `configuration.yaml` явно задано:

```yaml
lovelace:
  resource_mode: yaml
```

Home Assistant не позволява на интеграцията автоматично да променя списъка с ресурси. В този случай добавете ресурса ръчно:

```yaml
lovelace:
  resource_mode: yaml
  resources:
    - url: /api/smart_plug_multilevel_light/smart-plug-multilevel-light-card.js?v=0.10.9
      type: module
```

След промяната презаредете ресурсите на Lovelace или рестартирайте Home Assistant.

<a id="troubleshooting"></a>

## 🧯 Отстраняване на неизправности

<details>
<summary><b>❓Интеграцията не се показва в помощния списък</b></summary>
<br />

**Вероятна причина:** грешен начин или Home Assistant не беше рестартиран.

Проверете за наличност на файла:

```text
/config/custom_components/smart_plug_multilevel_light/manifest.json
```

След това направете твърд рестарт Home Assistant и проверете дневника за грешки `smart_plug_multilevel_light`.

<hr />
</details>

<details>
<summary><b>❓Списъкът с щепсели е празен</b></summary>
<br />

Интеграцията показва само обекти `switch`свързани с устройства, които също осигуряват включен сензор с `device_class: power`.

Настанете се **[Настройки → Устройства и услуги → Обекти](https://my.home-assistant.io/redirect/entities/)**:

* Принадлежат ли `switch` и сензор за мощност за едно устройство?
* Сензорът за захранване включен ли е?
* Правилно ли е посочено? `device_class`?
* Има ли числово състояние?

<hr />
</details>

<details>
<summary><b>❓Създаването спира със съобщение за липсващ сензор за мощност</b></summary>
<br />

След като изберете контакт, интеграцията проверява отново за наличие на сензор за мощност. Грешката възниква, ако сензорът е премахнат, деактивиран, преместен на друго устройство или вече не е такъв `device_class: power`.

<hr />
</details>

<details>
<summary><b>❓Лампата се показва като изключена, докато щепселът е включен</b></summary>
<br />

Проверете стойността на сензора за мощност. Виртуалната лампа се счита за изключена, когато `0 W`; когато е положителен, трябва да се определи като `on`.

<hr />
</details>

<details>
<summary><b>❓Лампата се показва като включена, докато е физически изключена</b></summary>
<br />

Проверете дали избраният сензор за мощност действително се показва `0 W`, когато лампата е изключена от собствен бутон.

<hr />
</details>

<details>
<summary><b>❓Открит е грешен режим</b></summary>
<br />

Проверете **🐞 Last measures** в настройки и атрибути `power_history`, `power_history_modes`, `power_sample_interval_seconds` и `selected_power_mode`. `power_history` показва последните записани стойности на мощността, `power_history_modes` — режим за всеки от тях според конфигурираните прагове, `power_sample_interval_seconds` е интервалът на периодична фиксация и `selected_power_mode` — текущ потвърден режим.

<hr />
</details>

<details>
<summary><b>❓Промените в режима не се показват веднага</b></summary>
<br />

Това е очаквано поведение. За да се предотвратят краткотрайни колебания на мощността, причиняващи фалшиво превключване, интегрирането променя режима само след като няколко последователни отчитания съответстват на един и същ нов режим.

Необходимият брой показания се задава от параметъра **Consecutive readings to switch** и по подразбиране `5`. Периодичните измервания се извършват веднъж в секунда и промените в състоянието на сензора се обработват незабавно. Ако по време на тестването стойността съответства на различен режим, обратното броене започва отново.

До потвърждение интеграцията поддържа предишния режим. Благодарение на това картата не превключва между режимите поради междинни стойности, които възникват при превключване на лампата, стабилизиране на нейната мощност или малки колебания в показанията на сензора.

Намаляването на броя на отчитанията ускорява показването на новия режим, но прави откриването по-чувствително към колебания в мощността.

<hr />
</details>

<details>
<summary><b>❓Картата не се показва в списъка</b></summary>
<br />

Автоматичната регистрация се извършва само след зареждане на поне един конфигуриран помощен обект и само в ресурсен режим `storage`. Методът за инсталиране на интеграция е чрез HACS или ръчно - това няма ефект.

Проверете **[Настройки → Панели → ⋮ → Ресурси](https://my.home-assistant.io/redirect/lovelace_resources/)**. URL адресът трябва да е там:

```text
/api/smart_plug_multilevel_light/smart-plug-multilevel-light-card.js?v=0.10.9
```

В режим YAML [добавете ресурс ръчно](#yaml-lovelace-mode).

<hr />
</details>

<details>
<summary><b>❓Старата карта все още се показва след актуализация</b></summary>
<br />

1. Напълно рестартирайте Home Assistant.
2. Обновете страницата, за да изчистите кеша на браузъра си.
3. Проверете номера на версията в URL адреса на ресурса на панела.
4. В мобилното приложение напълно затворете и отворете отново интерфейса.

<hr />
</details>

## 🗑 Премахване

1. Изтрийте всички създадени помощни обекти **Smart Plug Multi-Level Light** чрез **[Настройки → Устройства и услуги → Аксесоари](https://my.home-assistant.io/redirect/helpers/)**.
2. Премахнете интеграцията чрез HACS или ръчно изтрийте папката:

```text
/config/custom_components/smart_plug_multilevel_light
```

3. Проверете **[Настройки → Панели → ⋮ → Ресурси](https://my.home-assistant.io/redirect/lovelace_resources/)** и изтрийте ръчно ресурса на картата, ако остане:

```text
/api/smart_plug_multilevel_light/smart-plug-multilevel-light-card.js?v=0.10.9
```

4. Напълно рестартирайте Home Assistant.
5. Ако е необходимо, опреснете страницата, за да изчистите кеша на интерфейса.

## 📄 Лиценз

Този проект се разпространява под лиценз [MIT](https://github.com/LanKing/ha-smart-plug-multilevel-light/blob/main/LICENSE).

## 👋 Вашата подкрепа
[Ако този проект ви спести малко време, можете да подкрепите моите експерименти](https://github.com/sponsors/LanKing) (GitHub Sponsors). ☕🔧

<br />

> [!TIP]
> **Харесва ли ви това решение?** Разгледайте 🏠 <a href="https://github.com/LanKing/ha-tools/blob/main/docs/README_bg.md">другите ми Home Assistant продукти</a>
