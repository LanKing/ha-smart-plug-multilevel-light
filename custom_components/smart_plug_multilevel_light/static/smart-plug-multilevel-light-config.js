class SmartPlugModesEnhancer {
  static enhance() {
    ...
    const isOurSelector = (instance) => {
      ...
      return Boolean(
        object?.multiple &&
        fields?.name?.selector?.text !== undefined &&
        fields?.current?.selector?.number?.unit_of_measurement === "A"
      );
    };
    ...
    const showEditor = (instance, existing = null) => {
      ...
      body.innerHTML = `
        ...
        <div class="spml-editor">
          <div class="spml-field">
            <label class="spml-label" for="spml-name">${text(instance, "mode_name")}</label>
            <input id="spml-name" class="spml-input spml-name" type="text" autocomplete="off" />
          </div>
          <div class="spml-field">
            <div class="spml-threshold-head">
              <label class="spml-label" for="spml-current">${text(instance, "current_threshold")}</label>
              <div class="spml-measured">
                <span class="spml-measured-value"></span>
                <button type="button" class="spml-refresh">${text(instance, "refresh")}</button>
              </div>
            </div>
      ...
      footer.slot = "footer";
      footer.innerHTML = `
        <ha-button class="spml-cancel" appearance="plain" slot="secondaryAction">${text(instance, "cancel")}</ha-button>
        <ha-button class="spml-save" slot="primaryAction">${text(instance, editing ? "save" : "add")}</ha-button>
      `;
      ...
      const submit = () => {
        ...
        if (!name) {
          errorNode.textContent = text(instance, "required_name");
          nameInput.focus();
          return;
        }
        if (!Number.isFinite(current) || current < 0) {
          errorNode.textContent = text(instance, "required_current");
          currentInput.focus();
          return;
        }
    ...
    selector.prototype._addItem = async function (ev) {
      if (!isOurSelector(this)) return originalAdd.call(this, ev);
      ev.stopPropagation();
      const dialog = showEditor(this, null);
      dialog.addEventListener("spml-submit", (event) => {
        const next = Array.isArray(this.value) ? this.value.slice() : [];
        next.push(event.detail);
    ...
    selector.prototype._editItem = async function (ev) {
      if (!isOurSelector(this)) return originalEdit.call(this, ev);
      ev.stopPropagation();
      const item = ev.currentTarget.item;
      const index = ev.currentTarget.index;
      const dialog = showEditor(this, item);
      dialog.addEventListener("spml-submit", (event) => {
        const next = Array.isArray(this.value) ? this.value.slice() : [];
        next[index] = event.detail;
        next.sort((a, b) => Number(a.current) - Number(b.current));
        this.dispatchEvent(new CustomEvent("value-changed", {
          detail: { value: next }, bubbles: true, composed: true,
        }));
...
const tryEnhance = () => {
  if (customElements.get("ha-selector-object")) {
    SmartPlugModesEnhancer.enhance();
    return true;
  }
  return false;
};
...
if (!tryEnhance()) {
  customElements.whenDefined("ha-selector-object").then(() => SmartPlugModesEnhancer.enhance());
}
