class SmartPlugMultiLevelLightCardEditor extends HTMLElement {
  ...
  _t(key, fallback = key) {
    return window.SPML_I18N?.t(this._hass, key) ?? fallback;
  }

  _common(key, fallback) {
    return window.SPML_I18N?.common(this._hass, key, fallback) ?? this._hass?.localize?.(key) ?? fallback;
  }
  ...
  _schema() {
    return [
      ...
      },
      {
        type: "expandable",
        name: "interactions",
        title: this._t("interactions", "Interactions"),
        flatten: true,
        schema: [
          {
  ...
  _computeLabel(schema) {
    return ({
      entity: this._common("ui.common.entity", "Entity"),
      name: this._common("ui.common.name", "Name"),
      icon: this._common("ui.common.icon", "Icon"),
      show_mode: this._t("show_mode", "Show mode name"),
      show_percentage: this._t("show_percentage", "Show percentage"),
      always_show_icon_background: this._t("always_icon_bg", "Always show icon background"),
      icon_tap_action: this._t("icon_tap", "Icon tap behavior"),
    })[schema.name];
  }

  _computeHelper(schema) {
    return ({
      show_mode: this._t("helper_mode", "Shows the brightness mode detected from the configured current thresholds."),
      show_percentage: this._t("helper_pct", "Shows a synthetic brightness percentage derived from the mode position; it is not measured light output."),
      always_show_icon_background: this._t("helper_bg", "Keeps the circular icon background visible even when the icon has no action of its own."),
    })[schema.name];
  }
  ...
  _allDeep(selector, root = this.shadowRoot) {
    ...
    for (const element of root.querySelectorAll("*")) {
      if (element.shadowRoot) matches.push(...this._allDeep(selector, element.shadowRoot));
    }
    return matches;
  }
...
if (!customElements.get("smart-plug-multilevel-light-card-editor")) {
  customElements.define("smart-plug-multilevel-light-card-editor", SmartPlugMultiLevelLightCardEditor);
}
...
class SmartPlugMultiLevelLightCard extends HTMLElement {
  ...
  static getStubConfig(hass, entities) {
    ...
    return { entity: entity || "", show_mode: true, show_percentage: true, always_show_icon_background: false, icon_tap_action: { action: "more-info" } };
  }

  static getConfigElement() { return document.createElement("smart-plug-multilevel-light-card-editor"); }

  setConfig(config) {
    if (!config?.entity) throw new Error("entity is required");
    this.config = { show_mode: true, show_percentage: true, always_show_icon_background: false, icon_tap_action: { action: "more-info" }, ...config };
    this._syncTile();
  }

  set hass(hass) { this._hass = hass; this._syncTile(); }
  ...
  _stateContent(state) {
    ...
    if (this.config.show_mode && this.config.show_percentage !== false) return mode ? ["mode", "brightness_display"] : "brightness_display";
  ...
  _nativeConfig(state) {
    const cfg = { type: "tile", entity: this.config.entity, tap_action: { action: "toggle" }, hold_action: { action: "more-info" }, icon_tap_action: this.config.icon_tap_action || { action: "more-info" } };
  ...
  _visualState(realState) {
    ...
    const attributes = { ...realState.attributes, brightness_display: isOn ? `${Math.round(pct)}%` : undefined, supported_color_modes: ["hs"], color_mode: isOn ? "hs" : null };
    if (isOn) { attributes.brightness = brightness; attributes.rgb_color = visualRgb; attributes.hs_color = visualHs; }
    else { delete attributes.brightness; delete attributes.rgb_color; delete attributes.hs_color; }
    return { ...realState, attributes };
  }

  _visualHass(realState) {
    const visualState = this._visualState(realState);
    if (!visualState) return this._hass;
    return { ...this._hass, states: { ...this._hass.states, [this.config.entity]: visualState } };
  }

  _allDeep(selector, root) {
    if (!root) return [];
    const matches = [...root.querySelectorAll(selector)];
    for (const element of root.querySelectorAll("*")) if (element.shadowRoot) matches.push(...this._allDeep(selector, element.shadowRoot));
    return matches;
  }
  ...
  async _syncIconBackground(tile) {
    ...
    for (const tileIcon of this._allDeep("ha-tile-icon", tile.shadowRoot)) {
      ...
      container.classList.toggle("background", forceBackground || tileIcon.interactive === true);
    }
  }
...
if (!customElements.get("smart-plug-multilevel-light-card")) customElements.define("smart-plug-multilevel-light-card", SmartPlugMultiLevelLightCard);
...
if (!window.customCards.some((card) => card.type === "smart-plug-multilevel-light-card")) {
  window.customCards.push({
    ...
    description: "Native Home Assistant Tile Card for a smart-plug multi-level light.",
    preview: true,
    getEntitySuggestion: (hass, entityId) => {
      if (!entityId?.startsWith("light.")) return null;
      const state = hass.states[entityId];
      if (!state || !("configured_modes" in state.attributes)) return null;
      return { config: { type: "custom:smart-plug-multilevel-light-card", entity: entityId, show_mode: true, show_percentage: true, always_show_icon_background: false, icon_tap_action: { action: "more-info" } } };
    },
  });
}
